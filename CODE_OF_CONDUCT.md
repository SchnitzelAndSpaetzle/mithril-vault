# Code of Conduct

## Our Pledge

We as members, contributors, and maintainers of **MithrilVault** pledge to make participation in this project and our community a harassment-free experience for everyone—regardless of age, body size, visible or invisible disability, ethnicity, sex characteristics, gender identity and expression, level of experience, education, socio-economic status, nationality, personal appearance, race, caste, color, religion, or sexual identity and orientation.

We pledge to act and interact in ways that contribute to an open, welcoming, diverse, inclusive, and healthy community.

## Our Standards

Examples of behavior that contributes to a positive environment include:

- Demonstrating empathy and kindness toward other people
- Being respectful of differing opinions, viewpoints, and experiences
- Giving and gracefully accepting constructive feedback
- Taking responsibility for mistakes and learning from them
- Focusing on what is best for the overall community and project

Examples of unacceptable behavior include:

- The use of sexualized language or imagery, and sexual attention or advances of any kind
- Trolling, insulting, or derogatory comments, and personal or political attacks
- Public or private harassment
- Publishing others’ private information (such as physical or email addresses) without explicit permission
- Sharing, requesting, or threatening to share sensitive data, including:
  - passwords
  - private keys
  - vault contents
  - encryption keys
  - authentication tokens
- Any behavior that could reasonably be considered inappropriate in a professional or open-source setting

## Scope

This Code of Conduct applies within all community spaces related to **MithrilVault**, including but not limited to:

- GitHub issues
- Pull requests
- Code reviews
- GitHub Discussions
- Commit messages
- Project documentation

It also applies when an individual is officially representing the project in public spaces.

## Reporting

If you experience or witness unacceptable behavior, please report it as soon as possible.

**Primary contact:**
- 💬 **GitHub Discussions**  
  https://github.com/SchnitzelAndSpaetzle/mithril-vault/discussions

**Alternative reporting channel:**
- 🐛 **GitHub Issues**  
  https://github.com/SchnitzelAndSpaetzle/mithril-vault/issues

Reports will be handled with discretion. Confidentiality will be respected to the extent possible, while recognizing that limited information may need to be shared to investigate and respond appropriately.

### Security & Privacy Reports (Important)

Because **MithrilVault** is a password vault application, **security and privacy are critical**.

If your report involves:
- a security vulnerability
- accidental exposure of secrets
- cryptographic weaknesses
- leaked credentials or vault data

**Do NOT open a public issue.**

Instead, report it via:
- 🔒 **GitHub Security Advisories**  
  https://github.com/SchnitzelAndSpaetzle/mithril-vault/security/advisories

## Enforcement

Project maintainers are responsible for clarifying and enforcing our standards of acceptable behavior and will take appropriate and fair corrective action in response to any violations.

Maintainers who are the subject of a report will not participate in the enforcement decision for that report.

## Enforcement Guidelines

Community leaders will follow these guidelines when determining consequences:

### 1. Correction
**Impact:** Minor unprofessional or inappropriate behavior.

**Consequence:** A private or public written warning explaining why the behavior was inappropriate. A public apology may be requested.

### 2. Warning
**Impact:** Repeated or escalated violations.

**Consequence:** A formal warning with clear expectations for future behavior. Continued violations may result in temporary restrictions.

### 3. Temporary Ban
**Impact:** Serious or sustained violations.

**Consequence:** Temporary removal from participation in project spaces for a specified period.

### 4. Permanent Ban
**Impact:** Ongoing harassment, abuse, or severe misconduct.

**Consequence:** Permanent removal from all project-related spaces.

## Attribution

This Code of Conduct is adapted from the **Contributor Covenant, version 3.0**:
https://www.contributor-covenant.org/version/3/0/

Contributor Covenant is licensed under **CC BY-SA 4.0**:
https://creativecommons.org/licenses/by-sa/4.0/

For FAQs:
https://www.contributor-covenant.org/faq
