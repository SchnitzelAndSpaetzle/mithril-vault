...
|---------|--------|----------|
| `KdbxService::create()` method | Done | `src-tauri/src/services/kdbx.rs:274-313` |
...
| Basic tests | Done | `tests/kdbx_integration.rs` |
...
**File:** `src-tauri/src/services/kdbx.rs`
...
| File | Changes |
|------|---------|
| `src-tauri/src/services/kdbx.rs` | Update `create()`, add keyfile support, add default groups |
...
| `src-tauri/tests/kdbx_integration.rs` | Add new tests |
...
**Backend (Rust):**
- Added `DatabaseCreationOptions` struct in `models/database.rs` with configurable KDF parameters
