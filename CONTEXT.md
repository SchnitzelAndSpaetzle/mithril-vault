...
### Entry History
The ordered list of past full snapshots of an Entry, stored **natively** in the KDBX format (`keepass::Entry.history: Option<History>`, whose snapshots are reached through `History::get_entries()`, newest-first) — *not* a parallel MithrilVault store. An Entry with no recorded history has `history: None` (e.g. freshly imported or malformed entries); the listing treats `None` as an empty history rather than an error. The same substrate that Merge writes losing versions into (see Merge), so edit-history and merge-preservation share one mechanism and stay visible in any KeePass app. A snapshot is pushed *before* mutating, on **any change to the Entry's stored content or location**, matching KeePassXC — field edits (`update_entry`), tag changes (including bulk tag rename/delete, which therefore writes one snapshot per touched Entry), color (when added), Entry move between Groups, attachment add/delete, and custom-icon/Favicon changes. Only pure access-time bumps (reads) and Recycle-Bin transitions (send-to-trash and restore-from-trash, already reversible on their own) are excluded — the move rule covers user-initiated moves between real Groups only. Every Entry-mutating path funnels through a single snapshot chokepoint so coverage stays uniform as new mutable fields are added. Historical passwords and protected fields are never embedded in the history listing — they are fetched per-version on demand, mirroring `get_entry_password`. A version is addressed by its index in the newest-first list, guarded by a **stable per-version identity** so a concurrent edit can't silently retarget. The guard must not be `modified_at` alone: the pinned `keepass` `Times` type stores timestamps at *second* precision, so two snapshots created within the same second (e.g. a rapid edit while the history view is open) can share a `last_modification` and a position-only-plus-timestamp guard would pass while acting on the wrong version. The guard is instead a content fingerprint of the snapshot (hash over its fields/identity), verified against the addressed index before any view/restore.
_Avoid_: revision, audit trail (Audit is a separate concept).

### History Limit
The per-Entry cap on retained Entry Versions, configured **once per Vault** and stored in KDBX `Meta.history_max_items` — a property of the Vault file that travels with it, not an App Preference. Resolution: **absent → the default of 10** (newly created Vaults start from `Meta::default()` and never set this field, so absence must *not* mean unbounded password history); explicit **negative = unlimited**; `0` = disabled (no new snapshots; existing history pruned lazily on each Entry's next snapshot-producing *content* edit — reversible Recycle-Bin transitions leave it intact); positive N = keep newest N (oldest pruned on append). The byte cap (`Meta.history_max_size`) is preserved but **not** enforced in v1. Distinct from the read-only Database Config snapshot: History Limit is writable through its own vault-meta write surface.
...
For restore-of-attachments to work, the snapshot chokepoint must **explicitly retain referenced attachment blobs**: a pre-mutation entry clone does not register as a back-reference in the KDBX binary pool, so the existing attachment-delete path (which drops a blob once the *live* entry was its last reference) would otherwise GC a blob that only a history Version still needs. The chokepoint keeps (or copies) those blobs alive for as long as any Version references them.
