...
fn create_test_database() -> (KdbxService, TempDir, String) {
    ...
    service
        .create_database(&db_path_str, Some("testpass"), None, "Group CRUD", &options)
...
fn test_list_groups_success() {
    ...
    let result = service.list_groups(&db_path_str);
...
fn test_list_groups_database_not_open() {
    ...
    let result = service.list_groups("nonexistent.kdbx");
...
fn test_get_group_success() {
    ...
    let result = service.get_group(&db_path_str, root_group_id);
...
fn test_get_group_not_found() {
    ...
    let result = service.get_group(&db_path_str, "nonexistent-group-id");
...
fn test_get_group_database_not_open() {
    ...
    let result = service.get_group(db_path_str, "some-id");
...
fn test_create_group_in_root() {
    ...
    let result = service.create_group(&db_path_str, None, "New Group", None);
...
fn test_create_group_in_subgroup() {
    ...
    let parent = service
        .create_group(&db_path_str, None, "Parent Group", None)
        .expect("parent group");

    // Create child group
    let result = service.create_group(&db_path_str, Some(&parent.id), "Child Group", None);
...
fn test_create_group_with_icon() {
    ...
    let result = service.create_group(&db_path_str, None, "Icon Group", Some(5));
...
fn test_create_group_parent_not_found() {
    ...
    let result = service.create_group(&db_path_str, Some("nonexistent-id"), "Test", None);
...
fn test_create_group_database_not_open() {
    ...
    let result = service.create_group(db_path_str, None, "Test", None);
...
fn test_update_group_name() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Original Name", None)
        .expect("create group");
...
fn test_update_group_icon() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Test Group", None)
        .expect("create group");
...
fn test_update_group_both_fields() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Original", None)
        .expect("create group");
...
fn test_update_group_modified_timestamp_changes() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Test", None)
        .expect("create group");

    // Get the group to see its initial state
    let initial = service
        .get_group(&db_path_str, &group.id)
        .expect("get group");
...
fn test_rename_group_success() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Old Name", None)
        .expect("create group");
...
fn test_delete_group_soft_delete() {
    ...
    let group = service
        .create_group(&db_path_str, None, "To Delete", None)
        .expect("create group");
    let group_id = group.id.clone();

    // Delete without recursive, not permanent (moves to recycle bin)
    let result = service.delete_group(&db_path_str, &group_id, false, false);

    assert!(result.is_ok(), "Should successfully soft delete group");

    // The group should still exist (in recycle bin)
    let groups = service.list_groups(&db_path_str).expect("list groups");
    let root = &groups[0];
...
fn test_delete_group_permanent() {
    ...
    let group = service
        .create_group(&db_path_str, None, "To Delete Permanently", None)
        .expect("create group");
    let group_id = group.id.clone();

    // Delete permanently
    let result = service.delete_group(&db_path_str, &group_id, false, true);
    ...
    let get_result = service.get_group(&db_path_str, &group_id);
    assert!(
...
fn test_delete_group_recursive_with_children() {
    ...
    let parent = service
        .create_group(&db_path_str, None, "Parent", None)
        .expect("parent group");
    let _child = service
        .create_group(&db_path_str, Some(&parent.id), "Child", None)
        .expect("child group");

    // Delete recursively
    let result = service.delete_group(&db_path_str, &parent.id, true, true);
...
fn test_delete_group_non_recursive_fails_with_children() {
    ...
    let result = service.delete_group(&db_path_str, &parent.id, false, true);
...
fn test_delete_group_cannot_delete_root() {
    ...
    let result = service.delete_group(&db_path_str, &info.root_group_id, false, false);
...
fn test_delete_group_not_found() {
    ...
    let result = service.delete_group(&db_path_str, "nonexistent-id", false, false);
...
fn test_delete_group_database_not_open() {
    ...
    let result = service.delete_group(db_path_str, "some-id", false, false);
...
fn test_move_group_to_different_parent() {
    ...
    let group_a = service
        .create_group(&db_path_str, None, "Group A", None)
        .expect("group A");
    let group_b = service
        .create_group(&db_path_str, None, "Group B", None)
        .expect("group B");

    // Move Group A into Group B
    let result = service.move_group(&db_path_str, &group_a.id, Some(&group_b.id));
...
fn test_move_group_to_root() {
    ...
    let result = service.move_group(&db_path_str, &child.id, None);
...
fn test_move_group_cannot_move_root() {
    ...
    let target = service
        .create_group(&db_path_str, None, "Target", None)
        .expect("target");

    let result = service.move_group(&db_path_str, &info.root_group_id, Some(&target.id));
...
fn test_move_group_circular_reference_to_self() {
    ...
    let group = service
        .create_group(&db_path_str, None, "Group", None)
        .expect("create group");

    // Try to move group into itself
    let result = service.move_group(&db_path_str, &group.id, Some(&group.id));
...
fn test_move_group_circular_reference_to_descendant() {
    ...
    let grandchild = service
        .create_group(&db_path_str, Some(&child.id), "Grandchild", None)
        .expect("grandchild");

    // Try to move parent into its grandchild (circular)
    let result = service.move_group(&db_path_str, &parent.id, Some(&grandchild.id));
...
fn test_move_group_not_found() {
    ...
    let result = service.move_group(&db_path_str, "nonexistent-id", Some(&target.id));
...
fn test_move_group_target_not_found() {
    ...
    let result = service.move_group(&db_path_str, &group.id, Some("nonexistent-target"));
